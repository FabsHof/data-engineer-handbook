...
WITH teams_deduped AS (
    SELECT 
        *, 
        ROW_NUMBER() OVER (PARTITION BY team_id ORDER BY team_id) AS ROW_NUMBER
    FROM 
    teams
)
SELECT 
...
WITH deduped_game_details AS (
    SELECT 
        *, 
        ROW_NUMBER() OVER (PARTITION BY player_id, game_id ORDER BY player_id, game_id) AS ROW_NUMBER
    FROM game_details
)
SELECT 
